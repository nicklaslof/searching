...
constructor() {
        ...
        this.shaderProgram = new ShaderProgram(this.gl,`precision lowp float;attribute vec4 vp;attribute vec4 col;attribute vec2 aUV;uniform mat4 mvm;uniform mat4 vm;uniform mat4 pm;varying vec4 vc;varying vec2 uv;varying float zDist;void main() {gl_Position = pm * mvm * vp;vc = col;uv = aUV;zDist = gl_Position.z;}`,`precision lowp float;varying vec4 vc;varying vec2 uv;varying float zDist;uniform sampler2D s;void main() {vec4 col = texture2D(s, uv) * vc;gl_FragColor = vec4(col.rgb-(zDist/15.0),col.a);}`);
